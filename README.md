...
```
npx create-remix@latest --template mkromis/react-router-templates/blues-stack
```
...
To run tsx for you package manager it may be one of the following
...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create node-media-manager-7e23
  fly apps create node-media-manager-7e23-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app node-media-manager-7e23
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app node-media-manager-7e23-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'node-media-manager-7e23-staging' does not match app name in config file 'node-media-manager-7e23'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name node-media-manager-7e23-db
  fly postgres attach --app node-media-manager-7e23 node-media-manager-7e23-db

  fly postgres create --name node-media-manager-7e23-staging-db
  fly postgres attach --app node-media-manager-7e23-staging node-media-manager-7e23-staging-db
  ```
